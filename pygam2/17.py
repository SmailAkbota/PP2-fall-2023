...
game_over_image = pygame.image.load(os.path.join(sprite_folder, '5.jpg'))
...
background_image = pygame.transform.scale(background_image, (width, height/1.3))
bg_x = 0
...
def draw_images_near_scales():
    screen.blit(detail_images["GPA"], (width - 250, 85))
    screen.blit(detail_images["MOODSCALE"], (width - 250, 130))
...
def draw_health_bar():
    ...
    pygame.draw.rect(screen, GREEN, (width - 220, 110, health_bar_width, 20))

def show_score():
    score_text = font.render("Score: " + str(score), True, WHITE)
    screen.blit(score_text, (100, 110))

def draw_mood_bar(mood):
    mood_bar_width = int(200 * (mood / 100))
    pygame.draw.rect(screen, ORANGE, (width - 220, 150, mood_bar_width, 20))
while running:
    ...
    if state == "MENU":
        ...
        for detail in details:
            if detail["rect"].colliderect(pygame.Rect(player_x, player_y, player_size, player_size)):
                ...
                mood = max(0, mood - 0.5)
        if mood <= 0:
            health = max(0, health - 1)  # You can adjust the decrement value here
        else:
